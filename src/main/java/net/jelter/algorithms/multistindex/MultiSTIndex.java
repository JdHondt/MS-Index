...
import net.jelter.algorithms.mseg.*;
import net.jelter.algorithms.multistindex.segmentation.AdhocSegment;
...
public class MultiSTIndex extends Algorithm {
...
//    Boolean indicating if we want to use the index or not (in 2nd case we stop at root)
    ...
    public MultiSTIndex(boolean buildTree) {
...
//        Make folder index_caches if it does not exist
        ...
        return "index_caches/" + String.join("_",
                ...
                String.valueOf(seed),
                String.valueOf(mstSegmentMethod),
...
//        Insert all the segments into the R-tree
        ...
        if (mstSegmentMethod.equals(SegmentMethods.ADHOC)) {
...
//        Serialize the tree
    ...
    public ArrayList<TeunTuple3> kNN(int k, double[][] query) {
...
//        Derive an initial top k.
        final ArrayList<TeunTuple3> initialTopK = firstPass(k, query, queryMBR);
...
//        Derive the exact topK
        final ArrayList<TeunTuple3> finalTopK = secondPass(k, query, queryMBR, kThDistance);
    ...
    private ArrayList<TeunTuple3> firstPass(int k, double[][] query, MinimumBoundingRectangle queryMBR) {
        ...
        ArrayList<TeunTuple3> initialTopK = postFilter(query, groupedCandidateSegments, k, Double.POSITIVE_INFINITY);
        exhaustiveTime += System.currentTimeMillis() - start;

        return initialTopK;
    }

    private ArrayList<TeunTuple3> secondPass(int k, double[][] query, MinimumBoundingRectangle queryMBR, double threshold) {
...
//        Group the final candidates by time series
        ...
        start = System.currentTimeMillis();
        final ArrayList<TeunTuple3> finalTopK = postFilter(query, groupedFinalCandidates, k, threshold);
        exhaustiveTime += System.currentTimeMillis() - start;
...
//            Update the running threshold
     ...
     */
    public ArrayList<TeunTuple3> postFilter(double[][] query, Map<Integer, List<CandidateSegment>> groupedCandidateSegments,
