...
import static net.jelter.io.DataManager.data;
...
public class MVDSTreeOrg extends Algorithm implements Serializable {
    ...
    public MVDSTreeOrg() {
        indices = new DSTreeOrg[dimensions];
    ...
    public void buildIndex() {
        final TimeSeries[][][] subsequences = new TimeSeries[N][dimensions][];
        for (int n = 0; n < N; n++) {
        ...
        lib.getStream(IntStream.range(0, dimensions).boxed()).forEach(i -> indices[i].buildIndex(subsequences, i));
    }
...
//        Get thresholds for each dimension
        ...
        final double[][][] distanceCache = new double[N][dimensions][];
