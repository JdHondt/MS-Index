...
public final class Util {
     ...
     */
    public static MinimumBoundingRectangle mbr(Collection<? extends HasGeometry> items) {
        Preconditions.checkArgument(!items.isEmpty());
        MinimumBoundingRectangle result = null;
        for (final HasGeometry item : items) {
            MinimumBoundingRectangle r = item.geometry().mbr();
            if (result == null) {
                result = r;
            } else {
                result = result.add(r);
